/**
 * Integration tests for coinmarketcap-api-typescript.
 *
 * These tests make real HTTP requests to the CoinMarketCap API.
 * Set the COINMARKETCAP_API_KEY environment variable (or add a .env file)
 * before running these tests.
 *
 * Run: npx jest test.js
 */

const { default: CoinMarketCap } = require('./');

const API_KEY = process.env.COINMARKETCAP_API_KEY;
const itWithKey = API_KEY ? it : it.skip;
...
itWithKey('getTickers should have correct response structure and type', async () => {
  const client = new CoinMarketCap(API_KEY);
...
itWithKey('should get latest tickers', async () => {
  const client = new CoinMarketCap(API_KEY);
...
itWithKey('limit = 0 returns 5000 tickers', async () => {
  const client = new CoinMarketCap(API_KEY);
...
itWithKey('can pass in an array of currencies to convert for getTickers', async () => {
  const client = new CoinMarketCap(API_KEY);
...
test('passing in start and limit = 0 is not allowed in getTickers', async () => {
  const client = new CoinMarketCap(API_KEY);
  await expect(client.getTickers({ start: 2, limit: 0 })).rejects.toThrow(Error);
});

itWithKey('should get latest global', async () => {
  const client = new CoinMarketCap(API_KEY);
...
itWithKey('can pass in currencies in various ways to getGlobal', async () => {
  const client = new CoinMarketCap(API_KEY);
...
itWithKey('should get ID map', async () => {
  const client = new CoinMarketCap(API_KEY);
...
itWithKey('should get quotes', async () => {
  const client = new CoinMarketCap(API_KEY);
...
itWithKey('can pass in an array of currencies to convert for getQuotes', async () => {
  const client = new CoinMarketCap(API_KEY);
...
itWithKey('can pass in an array of IDs to id for getQuotes', async () => {
  const client = new CoinMarketCap(API_KEY);
...
test('must pass in id or symbol to getQuotes', async () => {
  const client = new CoinMarketCap(API_KEY);
  await expect(client.getQuotes()).rejects.toThrow(Error);
});

test('cannot pass in both id and symbol to getQuotes', async () => {
  const client = new CoinMarketCap(API_KEY);
  await expect(client.getQuotes({ id: 2, symbol: 'BTC' })).rejects.toThrow(Error);
});

itWithKey('should get metadata', async () => {
  const client = new CoinMarketCap(API_KEY);
